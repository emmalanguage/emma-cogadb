...
class UDFCompilerSpec extends FreeSpec with Matchers with CoGaDBSpec {
  ...
  "simple map udf with projected attributes" ignore withCoGaDB { implicit cogadb: CoGaDB =>
  ...
  "simple iteration with map udf" ignore withCoGaDB { implicit cogadb: CoGaDB =>
