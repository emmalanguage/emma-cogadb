...
class CoGaDB private(c: CoGaDB.Config) {
  ...
  val inst = Seq(c.home.resolve("bin/cogadbd").toString, c.startupFile.toString).run(true)
  ...
  try { Thread.sleep(1000) } catch { case _: Exception => () }
...
object CoGaDB {
  ...
  case class Config
  (
    home: Path = Paths.get(Option(System.getenv("COGADB_HOME")) getOrElse "/tmp/cogadb"),
    startupFile: Path = Paths.get(getClass.getResource("/cogadb/default.coga").toURI)
  )
